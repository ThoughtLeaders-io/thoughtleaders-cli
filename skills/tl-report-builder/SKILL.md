...
**Sample-row enrichment column names — read from the canonical schema, do NOT improvise.** When the rendered table needs columns beyond what the initial ES sample returned (typically `slug` for the hyperlink and a "last published" date), look up the column names in [`tl/references/postgres-schema.md` → `thoughtleaders_channel`](../tl/references/postgres-schema.md#thoughtleaders_channel-youtube-channels) before composing the PG query. The canonical column names there are authoritative; the agent has been observed guessing semantically-plausible alternatives (e.g. a `publication_date_max` form for the "most recent video" date when the actual column has a simpler name, or a `youtube_id` form when the actual column has a different prefix) and burning a 400 round-trip + an `information_schema.columns` fishing query to recover. **If you find yourself about to write a `SELECT ... FROM thoughtleaders_channel WHERE ...` query and you're not sure of a column name, consult the schema reference first** — do not guess and rely on the 400 to correct you, and do not fall back to `information_schema.columns` as the recovery path. The canonical column catalogue is one Markdown link away.
