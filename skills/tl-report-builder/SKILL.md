...
**MSN filtering — pinned to the FilterSet field.** When the user says *"msn only"* / *"our network"* / *"media selling network"*, the FilterSet patch is `{"filterset": {"msn_channels_only": true}}` — the boolean field documented in `references/intelligence_filterset_schema.json` (line 307) and listed as an intent override (line 379). Verbatim regression marker (same fitness/wellness run): the agent put `media_selling_network_join_date` into the saved FilterSet — but that name does not belong to any FilterSet field. The platform silently ignored the unknown field and the resulting report was unfiltered. **For FilterSet patches, the MSN entry is always `msn_channels_only: true`.** If you need an MSN predicate for validation-time `tl db pg` count queries, refer to the canonical schema at [`postgres-schema.md` → `thoughtleaders_channel`](../tl/references/postgres-schema.md#thoughtleaders_channel) — never assume column shapes from this file.
