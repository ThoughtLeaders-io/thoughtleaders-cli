...
- **Internal mechanism phrases** that describe HOW the skill works rather than WHAT the user is getting. Forbidden examples (verbatim regression markers — never say any of these to the user):
  ...
  - **`Campaign` / `Campaign #N` / `campaign_id`** — these are internal-jargon variants. The TL platform calls these **reports**. Always say "**TL report**" / "**report #N**" / "**report id**" in user-facing text (chat replies, save-success messages, the save tail). The user has never heard the term `Campaign`. *"Report saved. … (Campaign #23801)"* is a leak — write *"TL report saved. … (report #23801)"* instead.
  - **`reach` / "by reach" / "Reach"** — internal SQL column name. The user-facing term is **subscribers** (canonical mapping lives in `thoughtleaders-skills/tl-data/references/business-glossary.md`: *"AMs say subscribers, SQL says reach"*). Use `reach_from` / `reach_to` when emitting the FilterSet, but **always narrate as "subscribers"** — in sample-table column headers ("Subscribers", not "Reach"), in distribution stats ("By subscribers: 1M+ → 2", not "By reach: …"), in filter summaries ("only channels with 100K+ subscribers"), in takeaways. *"By reach: 1M+ → 2 · 100K–1M → 57 · 10K–100K → 128"* is a leak. **Sample-table column header rule (mandatory)**: a type-3 (channels) preview's first numeric column header MUST be literally `Subscribers`. The exact strings *`| Channel | Reach |`*, *`| Channel | Reach (subs) |`*, *`Reach (M)`*, *`Reach in millions`*, *`channel_reach`* are all forbidden as sample-table column headers — even when the underlying SQL uses `reach`. Real regression marker: a fitness/wellness preview emitted *`| Channel | Reach | Niche |`* despite the rule above; the column header MUST read `Subscribers`. The standard column triple is `Channel | Subscribers | Last published` — keep it unless the user explicitly asks for a different column.
...
If the user follows a save with *"add a filter for X"* / *"change the date range to Y"* / *"remove the language filter"* / *"also include Z"* — that's an **edit**, not a re-save. Run `tl reports update <report_id> - --config-file <path>` against the report you just created (the id is in the save response). The flow mirrors save: resolve a portable temp path → write the JSON patch to that file → invoke `tl reports update <id> - --config-file <that-exact-path>`. The `-` positional is the sentinel that tells the CLI to read the patch from the flag instead of the positional arg. Patch shape is whatever FilterSet fields you're changing — e.g., `{"filterset": {"msn_channels_only": true}}`. Use the file-transport invocation so apostrophes / dollar signs / backticks in the patch don't blow up shell quoting (same reasoning as save mode).
...
**MSN filtering — pinned to the FilterSet field.** When the user says *"msn only"* / *"our network"* / *"media selling network"*, the FilterSet patch is `{"filterset": {"msn_channels_only": true}}` — the boolean field documented in `references/intelligence_filterset_schema.json` (line 307) and listed as an intent override (line 379). Verbatim regression marker (same fitness/wellness run): the agent put `media_selling_network_join_date` into the saved FilterSet, but that name belongs to a column you can query against via `tl db pg`, not to a field the FilterSet accepts. The platform silently ignored the unknown field; the resulting report was unfiltered. The CLI behavior here is a hard split:

- **For report patches** (`tl reports update`): the FilterSet has exactly one MSN entry — `msn_channels_only: true`. Always this; no other path.
- **For validation counts** (`tl db pg` / `tl db es` during Phase 3): you query the channel table directly with `media_selling_network_join_date IS NOT NULL` as the MSN-member predicate. This is COUNT-time only — never goes into a saved FilterSet. The column lives in [`postgres-schema.md` → `thoughtleaders_channel`](../tl/references/postgres-schema.md#thoughtleaders_channel) (canonical home; refer there for column types and other channel fields).

These are two different surfaces for the same business concept; the regression was confusing the validation-time column name with the FilterSet field. Pin the FilterSet patch as `msn_channels_only: true` always, and only use the column form for `tl db pg` count queries.
...
> - **Default to preview**, then close the reply with one line: *"If you want this saved as a TL report you can come back to, just say save."* — **never** *"saved as a campaign"* (see rule 6).
...
`sort` is a `FilterSet` field, so **Phase 2 picks the value when composing the FilterSet** — defaulting to the type's pagination default (`-reach` for type 3, `-views` for type 1, `-doc_count` for type 2, `-purchase_date` / `-send_date` for type 8 per axis branching) unless the user's intent overrides (e.g., outreach intent on type 3 → `-publication_date_max`).
