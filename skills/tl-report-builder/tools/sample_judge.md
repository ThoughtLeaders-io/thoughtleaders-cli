...
3. **`DB_SAMPLE`** — array of up to 10 row objects. Shape depends on `REPORT_TYPE`:
   ...
   | **2 (BRANDS)** | `{ id, brand_name, mentions_count, channels_count?, last_mention_date? }` — `id` is the brand ID (agg bucket key). `brand_name` is populated by the orchestration via a PG batch lookup against `thoughtleaders_brand.id` (brand names aren't in ES). `mentions_count` comes from the agg bucket's `doc_count` directly. `channels_count` and `last_mention_date` are optional (require additional per-brand ES calls because `tl db es` accepts only one aggregation per request — see SKILL.md Step 2.V1 type-2 section). When absent, judge from `brand_name` + `mentions_count` alone. | `brand_name` |
