...
6. **Composability**: a single create_report response can have `cross_references` entries AND `similar_to_channels` (in `filters_json`) AND `exclude_brand_names` (in filterset) simultaneously — they are independent. Don't collapse them into a `multi_step_query`.
