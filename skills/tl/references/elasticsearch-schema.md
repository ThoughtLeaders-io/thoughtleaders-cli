...
- **`highlight` fragments are returned only when you pass `--highlight`** (`tl db es <body> --highlight`). Without the flag, result rows carry only `_id`, `_score`, and the `_source` fields, even if the body has a `highlight` clause. With it, each row also carries the ES highlight block. Fragments of `transcript` come back as timed-text XML fragments (`<text start="…">` cues), so strip tags before windowing. Premium-field billing applies to shipped fragments, and free-tier orgs get gated fields stripped from highlight blocks.
