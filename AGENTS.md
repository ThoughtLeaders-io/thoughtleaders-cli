...
- **PKCE + Auth0**: Browser-based login with localhost callback server (`login.py`). Auth0 is reached on `auth.thoughtleaders.io`, the custom domain the web platform and Chrome extension also sign in on, so the three surfaces share one SSO cookie. On success the callback redirects the browser to the platform's `/signin?go=1&from=cli` so the web session (and the extension, which adopts it) follow. `tl auth logout` calls the platform's sign-out (which ends the session for the extension and other CLIs) and then drives the platform's `/logout` in the browser — with `web_only=1` once the sign-out is recorded, so the page only ends the browser's session and cannot record a later sign-out that would refuse a fresh sign-in; without it when the call failed, so the page is the fallback that records it. `--local` opts out of both. Every bearer request carries `X-TL-Signed-In-At`, the time of the interactive sign-in, so the platform can refuse a session ended elsewhere even after the access token has been refreshed. A 401 with `code: signed_out` makes the client drop its credentials rather than refresh (`forget_session`).
