...
Data access uses the shared CLI/MCP provider. No database credentials
are used.
"""
...
OUT_DIR = Path(os.environ.get("TL_MCP_SESSION", "/tmp"))
...
def collect(ref: str) -> dict:
    ...
    state = {
        "comment_collection_complete": True,
        "ref": ref,
        "channel": ch,
    ...
    return {
        ...
        "next": (
            ...
            f"{ch.get('language')}'). Run two independent complete classification passes and save each JSON reply, then run: "
            "analyze_channel.py --finalize <state_path> <pass1.json> <pass2.json>"
        ),
    }
...
def finalize(state_path: str, llm_paths: list[str]) -> dict:
    ...
    if state.get("comment_collection_complete") is not True:
        raise ValueError("Incomplete comment evidence: recollect with verified comment retrieval.")
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="state.json followed by two complete classifier-output files "
    ...
    try:
        if args.finalize:
            if len(args.finalize) < 1:
                ap.error("--finalize needs STATE")
        ...
        sys.exit(getattr(exc, "exit_code", 3))

    except (comment_scraper.CommentRetrievalError, ValueError, OSError, tl_cli.DataError) as exc:
        print(json.dumps({"error": "incomplete_evidence", "message": str(exc)}))
        sys.exit(getattr(exc, "exit_code", 3))
