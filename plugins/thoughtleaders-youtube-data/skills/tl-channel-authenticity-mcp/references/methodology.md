<!-- Generated verbatim from the canonical skill body; do not edit. -->

# Reading this workflow through MCP

Read [MCP runtime](mcp-runtime.md) first. Its execution instructions govern this package. The canonical workflow below retains CLI examples as script/argument notation and legacy agent names; do not execute CLI commands or install/authenticate the CLI. Use the packaged Python scripts through the MCP runner, and the bundled agent prompts. Persistence and optional external integrations require separately available tools. Canonical source: `skills/tl-channel-authenticity/SKILL.md`.
