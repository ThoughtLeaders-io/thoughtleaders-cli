...
def run_es(body):
    """Fetch the full result; candidate failures stay distinct from no matches."""
    try:
        return tl_data.query("es", body, timeout=PROBE_TIMEOUT)
...
def main():
    ...
    bodies = [build_body(cand, fields=fields, level=args.level, samples=samples,
    ...
    tl_data.prefetch([{"engine": "es", "query": body,
                       "include_highlight": False, "pricing": False}
                      for body in bodies])
    results, failed = [], []
